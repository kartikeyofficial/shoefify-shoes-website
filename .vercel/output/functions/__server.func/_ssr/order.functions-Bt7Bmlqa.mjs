import { c as createServerRpc, a as connectDB, g as getServerUser, O as Order } from "./auth.server-DFBqw7e2.mjs";
...
const getMyOrders = createServerFn({
  ...
  const orders = await Order.find({
    ...
    createdAt: -1
  }).populate("items.product_id", "images slug").lean();
  const mappedOrders = orders.map((o) => {
    const itemsWithImagesAndSlug = o.items.map((item) => ({
      ...item,
